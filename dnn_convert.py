...
def dnn_convert(file_path, file_format, dest_path, variant):
    if file_format == 'nnet':
        if variant == 'python':
...
def main():
    ...
    parser.add_argument('--file_path', default='./resources/nnet/1_mnist10x10.nnet')
    parser.add_argument('--file_format', default='nnet')
    parser.add_argument('--dest_path', default='./fol/dnn/1_mnist10x10')
    parser.add_argument('--variant', default='python')

    config = parser.parse_args()


    dnn_convert(config.file_path, config.file_format, config.dest_path, config.variant)
